You are working on a hard mathematical problem. Do not claim a full solution unless every step is rigorous.

Target for this iteration:
Prove an exact formula for F_c(n) when c >= 1, including floors and parity.

Local context:
SOURCE SUMMARY:
# Source Summary for Erd艖s Problem 361
...
The page lists the problem as open and cites `[ErGr80,p.59]`, meaning Erd艖s and Graham, *Old and New Problems and Results in Combinatorial Number Theory* (1980), page 59.
...
- Erd艖s Graham subset sums page 59
...
```text
Work on Erd艖s Problem 361. Prove a rigorous exact formula for F_c(n) when c >= 1, including all floor and parity issues. Do not claim anything about 0<c<1. After proving, list the weakest step and possible counterexamples.
```
...
Produce:
1. A precise claim.
2. A rigorous proof attempt.
3. A list of all floor, parity, endpoint, and large-n obligations.
4. The weakest step.
5. A suggested narrower next target if the proof fails.
